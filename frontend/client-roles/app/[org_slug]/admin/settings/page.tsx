...
const PMSettings = () => {
  ...
  const [activeSessions] = React.useState([
    { id: 1, device: "Windows PC - Chrome", location: "New York, US", lastActive: "Now", current: true },
    { id: 2, device: "iPhone 15 - Safari", location: "New York, US", lastActive: "2 hours ago", current: false },
    { id: 3, device: "MacBook Pro - Firefox", location: "Boston, US", lastActive: "3 days ago", current: false },
  ]);
        ...
        if (project) {
          ...
          const [healthRes, alertsRes] = await Promise.all([
            projectManagerService
              ...
              .catch(() => ({ data: null })),
            projectManagerService
              .getSystemAlerts(project.id)
              .catch(() => ({ data: [] })),
          ]);

          console.log("[PM Settings] Active Project:", project);
          console.log("[PM Settings] Endpoints:", [
            `/api/v1/project-manager/dashboard/strategic-health/${project.id}/`,
            `/api/v1/project-manager/dashboard/system-alerts/${project.id}/`,
          ]);
          console.log("[PM Settings] Strategic Health:", healthRes.data);
          console.log("[PM Settings] PM Auto-Setup Configuration loaded");
  ...
  const safetyScore =
    healthData?.safety_score ?? healthData?.site_safety_score ?? null;
    ...
    },
    {
      ...
      value:
        budgetVariance != null
          ...
          : "—",
      color:
        budgetVariance != null && budgetVariance < 0 ? "#dc2626" : "#0166B0",
    },
    {
      label: "Quality",
    ...
    <div className="pb-24 text-[#021422]">
      ...
      <div className="bg-[#021422] text-white p-4 px-8 flex justify-between items-center text-sm border-b border-gray-800">
        <div className="font-bold tracking-widest uppercase">
          PM QUICK COMMAND BAR
        </div>
        <div className="flex gap-4">
          <button className="bg-white/10 px-6 py-1 rounded uppercase font-bold text-xs hover:bg-white/20 transition-colors">
            Minimize
          </button>
          <button className="bg-white/10 px-6 py-1 rounded uppercase font-bold text-xs hover:bg-white/20 transition-colors flex items-center gap-1">
            <Lock size={12} /> Pin
          </button>
        </div>
      </div>
      ...
      <div className="p-8 max-w-7xl mx-auto space-y-8">
            ...
            <div className="space-y-6 pt-6 border-t border-gray-100">
              ...
              <div className="space-y-4">
                {systemAlerts.length > 0 ? (
                  systemAlerts.slice(0, 5).map((alert: any, idx: number) => (
                    <div
                      key={idx}
                      className="text-sm font-bold uppercase text-gray-700 pl-4"
                    >
                      {idx + 1}.{" "}
              ...
              <div className="flex gap-2">
                <button className="bg-[#021422] text-white px-10 py-2 rounded text-sm font-bold uppercase transition-colors hover:bg-gray-800">
                  View All
                </button>
                <button className="bg-[#0166B0] text-white px-10 py-2 rounded text-sm font-bold uppercase transition-colors hover:bg-blue-700">
                  Acknowledge All
                </button>
              </div>
            </div>
        ...
        {/* Quick Actions */}
        <div className="text-gray-900 font-bold text-base uppercase mb-4">
          Quick Actions:
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            return (
              <button
                key={idx}
                className={`flex items-center justify-center gap-2 p-4 rounded font-bold text-sm uppercase transition-colors ${
        ...
        {/* One-Click Reports */}
        <div className="text-gray-900 font-bold text-base uppercase mb-4 pt-4">
          One-Click Reports:
        </div>
        ...
        <div className="bg-white border rounded-xl p-12 space-y-8 flex flex-col items-center justify-center">
          <div className="space-y-6 w-full max-w-lg text-center font-bold text-base text-gray-800">
            <div className="hover:text-[#0166B0] cursor-pointer transition-colors">
              &quot;AI, what&apos;s my top priority?&quot;
            </div>
            <div className="hover:text-[#0166B0] cursor-pointer transition-colors">
              &quot;AI, draft client update&quot;
            </div>
            <div className="hover:text-[#0166B0] cursor-pointer transition-colors">
              &quot;AI, analyze schedule risk&quot;
            </div>
          </div>
        </div>
        ...
        {/* Account Security */}
        <div className="text-gray-900 font-bold text-base uppercase mb-4 pt-8">
          Account Security:
        </div>
        <div className="space-y-4 pb-12">
          {/* Change Password */}
          <div className="flex items-start justify-between p-4 bg-gray-50 rounded-lg border border-gray-200">
            <div className="flex items-start gap-3">
              <div className="bg-[#0166B0]/10 p-2 rounded-lg">
                <Key size={20} className="text-[#0166B0]" />
              </div>
              <div>
            ...
            <button
              ...
              className="bg-[#021422] text-white px-4 py-2 rounded text-sm font-bold uppercase transition-colors hover:bg-gray-800 whitespace-nowrap"
            >
          ...
          {showPasswordForm && (
            <form onSubmit={handlePasswordSubmit} className="p-4 bg-white border border-gray-200 rounded-lg space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#0166B0]"
              ...
              <div className="flex gap-2 pt-2">
                <button
                  type="submit"
                  className="bg-[#0166B0] text-white px-6 py-2 rounded text-sm font-bold uppercase transition-colors hover:bg-blue-700"
                >
                  Save Password
                </button>
                <button
                  ...
                  className="bg-gray-200 text-gray-700 px-6 py-2 rounded text-sm font-bold uppercase transition-colors hover:bg-gray-300"
                >
                  Cancel
                </button>
              </div>
          ...
          <div className="flex items-start justify-between p-4 bg-gray-50 rounded-lg border border-gray-200">
            <div className="flex items-start gap-3">
              <div className="bg-[#0166B0]/10 p-2 rounded-lg">
                <Smartphone size={20} className="text-[#0166B0]" />
              </div>
              <div>
            ...
            <button
              ...
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                twoFactorEnabled ? "bg-[#0166B0]" : "bg-gray-300"
              }`}
            >
              <span
