    ...
    <div className="h-screen bg-[#E3E3E3] text-slate-900 font-sans flex flex-col overflow-hidden">
      ...
      <div className="bg-white border-b border-gray-100 px-8 py-5 flex flex-col md:flex-row md:items-center justify-between gap-4 shrink-0">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button onClick={() => setIsExportModalOpen(true)} className="px-4 py-2 bg-white border border-gray-100 text-slate-700 hover:bg-white rounded-lg text-sm font-bold transition-colors flex items-center gap-2 shadow-sm">
      ...
      <div className="max-w-[1400px] mx-auto px-6 mt-6 mb-6 flex-1 w-full flex flex-col overflow-hidden gap-6">
        ...
        {/* Contracts Table */}
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm flex-1 overflow-hidden flex flex-col min-h-0">
          <div className="overflow-y-auto custom-scrollbar flex-1">
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-white/80 text-slate-500 text-[11px] uppercase font-bold tracking-wider border-b border-gray-100 sticky top-0 z-10 backdrop-blur-md">
                <tr>
              ...
              <tbody className="divide-y divide-slate-100">
                {contractsData.map((contract) => (
                  <tr key={contract.id} className="hover:bg-white/50 transition-colors group">
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm" onClick={() => setIsExportModalOpen(false)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <div className="flex gap-3">
                  ...
                  <label className="flex-1 border border-gray-100 rounded-lg p-3 flex items-center gap-2 cursor-pointer hover:bg-white transition-colors">
            ...
            <div className="p-4 border-t border-slate-100 bg-white flex justify-end gap-2">
              <button onClick={() => setIsExportModalOpen(false)} className="px-4 py-2 bg-white border border-gray-100 text-slate-700 font-medium rounded-lg hover:bg-white transition-colors text-sm">Cancel</button>
      ...
      <div className={`fixed inset-y-0 right-0 z-[60] w-full max-w-lg bg-white shadow-2xl transform transition-transform duration-300 ${isNewContractOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="h-full flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            <div className="border-2 border-dashed border-gray-100 rounded-xl p-6 flex flex-col items-center justify-center text-center hover:bg-white transition-colors cursor-pointer bg-white/50">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" placeholder="e.g. Phase 2 MEP Works" className="w-full border border-gray-100 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1">Vendor / Subcontractor</label>
                <input type="text" placeholder="Enter company name" className="w-full border border-gray-100 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1">Contract Value (₦)</label>
                  <input type="text" placeholder="0.00" className="w-full border border-gray-100 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                </div>
                <div>
          ...
          <div className="p-4 border-t border-slate-100 bg-white flex justify-end gap-3 shrink-0">
            <button onClick={() => setIsNewContractOpen(false)} className="px-4 py-2 bg-white border border-gray-100 text-slate-700 font-bold rounded-lg hover:bg-slate-100 transition-colors text-sm">Cancel</button>
