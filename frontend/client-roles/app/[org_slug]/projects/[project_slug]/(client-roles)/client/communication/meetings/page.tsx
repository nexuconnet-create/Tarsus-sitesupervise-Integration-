        ...
        <div className="flex flex-col sm:flex-row gap-4 shrink-0">
          <div className="flex bg-white rounded-xl border border-gray-100 p-1 shadow-sm shrink-0">
            <button 
              ...
              className={`px-6 py-2 rounded-lg text-sm font-bold transition-colors ${view === 'upcoming' ? 'bg-[#021422] text-white' : 'text-slate-500 hover:text-slate-700 hover:bg-white'}`}
            >
              Upcoming
            </button>
            <button 
              onClick={() => setView('past')}
              className={`px-6 py-2 rounded-lg text-sm font-bold transition-colors ${view === 'past' ? 'bg-[#021422] text-white' : 'text-slate-500 hover:text-slate-700 hover:bg-white'}`}
            >
              Past Meetings
            </button>
          </div>
          ...
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full pl-11 pr-4 py-3 border border-gray-100 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white shadow-sm h-full"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          <div className="grid gap-4">
            {displayedMeetings.map(meeting => (
              <div key={meeting.id} className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm hover:shadow-md transition-shadow flex flex-col sm:flex-row sm:items-center justify-between gap-4 group">
                ...
                <div className="flex items-center gap-2 self-start sm:self-center shrink-0">
                  {view === 'upcoming' ? (
                    <>
                      ...
                      <button className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-colors border border-transparent hover:border-gray-100">
                        <CalendarPlus size={18} />
                      </button>
                    </>
                  ) : (
                    <>
                      <button className="px-4 py-2 bg-white text-slate-700 hover:bg-slate-100 border border-gray-100 rounded-lg text-sm font-bold transition-colors flex items-center gap-2">
                        View Summary
                      </button>
                    </>
                  )}
