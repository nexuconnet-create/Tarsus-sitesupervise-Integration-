...
const STATUS_OPTIONS: { value: "present" | "absent"; label: string; color: string; bgColor: string }[] = [
  { value: "present", label: "Attended", color: "text-green-600", bgColor: "bg-green-50" },
...
export default function EditRecordModal({
  ...
  // The backend only accepts ABSENT/ON_LEAVE on write; PRESENT/LATE/EARLY are
  // computed from check_in/check_out. The UI therefore offers only "attended"
  // (maps to no writable status + optional times) or "absent".
  ...
  const handleSave = () => {
    if (!isAbsent && checkIn && checkOut && checkIn > checkOut) {
      toast.error("Check out time must be after check in time");
      return;
    }
    onSave({ status: status as AttendanceStatus, checkIn, checkOut, notes });
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">{worker.name} — {dateDisplay}</p>
          </div>
          <button
            onClick={onClose}
          ...
          <div>
            ...
            {!isAbsent && (
              <p className="text-[10px] text-gray-400 mt-1.5">
                Present / Late / Early are computed from the clock times below.
              </p>
            )}
          </div>

          <div className={`grid grid-cols-2 gap-4 ${isAbsent ? "opacity-50 pointer-events-none" : ""}`}>
            <div>
              <label className="text-[10px] font-bold uppercase tracking-wider text-gray-500 block mb-2">
                Check In
              </label>
              <input
                ...
                disabled={isAbsent}
                className="w-full px-3 py-2 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#007AFF] disabled:bg-gray-100"
              />
            </div>
            <div>
              <label className="text-[10px] font-bold uppercase tracking-wider text-gray-500 block mb-2">
