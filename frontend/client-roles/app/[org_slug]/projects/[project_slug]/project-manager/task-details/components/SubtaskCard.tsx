...
export default function SubtaskCard({ subtask, onApprove, onReject }: SubtaskCardProps) {
  ...
  // Resolve the actioned-by name — API data first, fall back to logged-in user
      ...
      >
          ...
          <div className="flex items-center gap-2">
            {subtask.status === "pending" && (
              <div className="flex items-center gap-1">
                <button
                  onClick={(e) => { e.stopPropagation(); onApprove?.(subtask.id); }}
        ...
        <div className="flex items-center gap-4 mt-3 text-xs text-gray-500 flex-wrap">
          <span className="flex items-center gap-1">
            ...
            <span className="font-medium text-[#021422]">{subtask.requestedBy}</span>
          </span>
          <span className="flex items-center gap-1">
            <Clock size={12} />
          ...
          {subtask.status === "approved" && (
            <span className="flex items-center gap-1 text-green-600">
              ...
              <span className="font-medium">{actionedByName}</span>
            </span>
          )}
                      ...
                      {subtask.materials.map((material) => (
                        <div key={material.id} className="flex items-center justify-between text-xs">
                          ...
                          <span className="text-gray-500">
                            Qty: {material.quantity}{material.unit ? ` ${material.unit}` : ""}
                          </span>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="space-y-1">
                      {subtask.equipment.map((equipment) => (
                        <div key={equipment.id} className="flex items-center justify-between text-xs">
                          ...
                          {equipment.quantity ? (
                            <span className="text-gray-500">Qty: {equipment.quantity}</span>
                          ) : (
                            <span className={`px-1.5 py-0.5 rounded text-xs ${
                              equipment.status === "on_site"
