...
import { useMiscExpenseStore } from "@/store/miscExpenseStore";
...
const FinancialCommand = () => {
    ...
    // Miscellaneous-expense rollup (mock-backed until backend EVM endpoint ships)
    const miscExpenses = useMiscExpenseStore((s) => s.expenses);
    const miscTotals = React.useMemo(() => {
        return miscExpenses.reduce(
            (acc, e) => {
                acc.pv += e.amountPlanned;
                acc.ev += e.earnedValue;
                acc.ac += e.amountActual;
                return acc;
            },
            { pv: 0, ev: 0, ac: 0 },
        );
    ...
    return (
        <div className="pb-24">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white py-7 px-4">
                <div className="flex items-center gap-3"><BackButton /><div className="text-2xl font-bold text-[#021422]">Financial Command Center — {activeProject.name}</div></div>
                        ...
                        <div className="border-t pt-6">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-base font-medium mb-4">
                                <div className="space-y-1">
                                    <div className="text-gray-500 text-xs uppercase">Misc Planned (PV)</div>
                                    <div>{formatNaira(miscTotals.pv)}</div>
                                </div>
                                <div className="space-y-1">
                                    <div className="text-gray-500 text-xs uppercase">Misc Earned (EV)</div>
                                    <div>{formatNaira(miscTotals.ev)}</div>
                                </div>
                                <div className="space-y-1">
                            ...
                            <a href="misc-budget" className="inline-block mt-3 text-sm font-bold text-[#0166B0] hover:underline">
                                View full Miscellaneous Budget &rarr;
                            </a>
                        </div>
                    </div>
                </DashboardSection>
