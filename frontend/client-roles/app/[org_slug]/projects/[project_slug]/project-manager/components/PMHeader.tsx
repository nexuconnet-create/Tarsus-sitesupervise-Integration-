...
function getRoleAbbreviation(role: string): string {
  ...
  return ROLE_ABBREVIATIONS[normalized] ?? role.replace(/_/g, " ").split(" ").map((w) => w[0]).join("").toUpperCase();
}
...
export default function PMHeader({
  ...
  const fullName =
    user?.first_name && user?.last_name
      ? `${user.first_name} ${user.last_name}`
      : user?.fullname || user?.name || user?.username || "Adebayo";

  const rawRole = user?.role_name || user?.role || "Project Manager";
  ...
  return (
    <div className="flex items-center justify-between bg-white py-5 px-4 border-b border-gray-100">
      <div className="flex items-center gap-3">
        ...
        {showDateTime && (
          <p className="text-xs font-medium text-gray-500">{formatDateTime(now)}</p>
        )}
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        <div className="text-right">
          <p className="text-sm font-bold text-[#021422] leading-tight">
            {fullName}
            {specialization && (
              <span className="font-normal text-gray-700 italic"> ({specialization})</span>
            )}
          </p>
