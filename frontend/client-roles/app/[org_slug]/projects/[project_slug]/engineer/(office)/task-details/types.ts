...
/** Per-crew-line billing basis for daily_worker assignments. Task-level default
 * only pre-fills new lines — the per-line value is authoritative for cost. */
export type RateType = "daily" | "hourly";

export interface CrewAssignment {
  crewId: string;
  workerType?: WorkerType;
  /** The rate value — daily_rate when rateType is "daily", hourly_rate when "hourly". */
  price?: number;
  rateType?: RateType;
}
...
export interface Task {
  ...
  // Milestone name — populated directly from list API response (no lookup needed)
  milestoneName?: string;
  // Checklist count — populated for list items (no full taskTracker loaded yet)
  ...
  reschedule_approved_by_pm?: boolean;
  // Pending edit / reschedule request (submitted by engineer, approved by PM)
  pendingRescheduleRequest?: PendingRescheduleRequest;
  milestoneId: string;
  // Concrete test results uploaded flag
  concreteTestResultsUploaded?: boolean;
  // Approval gate (v1-sholly) — distinct from `status` (schedule health) and
  // `queue` (workflow). Drives the PM approve/reject controls.
...
export interface EquipmentResource {
  id: string;
  name: string;
  status: "on_site" | "off_site" | "maintenance" | "reserved" | "pending" | "requested";
  location?: string;
  operator?: string;
  isNew?: boolean;
  category?: string;
  ...
  quantity?: string;
  plannedDays?: number;
  // Demurrage (RENTED/LEASED only — backend rejects on OWNED equipment).
  plannedIdleDays?: number;
...
export type MiscCategory = "utility" | "transport" | "permit" | "levy" | "sundry" | "other";
...
// ─── Files & Documents Page Types ───────────────────────────────────────────

export type DocumentCategory = "task" | "attendance" | "schedule";
...
export interface ProjectFileDocument {
  ...
  new_end_date?: string;
  reschedule_reason?: string;
  // Schedule fields (for non-rescheduled tasks)
  scheduled_start_date?: string;
  scheduled_end_date?: string;
  // Versioning
  version_type?: VersionType;
  version_number?: number;
  // Extended task detail (for rich preview)
  progress?: number;
  description?: string;
  crews?: { id: string; name: string; trade: string; size: number }[];
  rescheduled_by?: string;
  reschedule_approved_by_pm?: boolean;
  pm_approved?: boolean;
  risk?: string;
  // Full task data snapshot at time of this version (for preview)
  task_snapshot?: Partial<import("@/lib/types").Task>;
}
...
export interface AttendanceReportFile {
  ...
  report_date: string;
  crew_manager_name: string;
  present_count: number;
  absent_count: number;
  late_count: number;
  linked_task_wps?: string[];
  created_at: string;
}

export interface ScheduleReportFile {
  id: string;
...
export type VersionType = "created" | "updated" | "rescheduled" | "completed";

export const VERSION_TYPE_CONFIG: Record<VersionType, { bg: string; text: string; dot: string; label: string }> = {
  created:     { bg: "bg-blue-100",   text: "text-blue-800",   dot: "bg-blue-500",   label: "Created" },
  updated:     { bg: "bg-yellow-100", text: "text-yellow-800", dot: "bg-yellow-400", label: "Updated" },
  rescheduled: { bg: "bg-orange-100", text: "text-orange-800", dot: "bg-orange-500", label: "Rescheduled" },
