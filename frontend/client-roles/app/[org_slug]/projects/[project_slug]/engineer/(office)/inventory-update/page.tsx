...
import type { PrefilledLineItem } from "@/app/[org_slug]/projects/[project_slug]/_components/inventory/CreatePOModal";
...
const MOCK_INVENTORY_POS: InventoryPO[] = [
  {
    uuid: "mock-po-001",
    poNumber: "PO-2026-0042",
    supplierName: "Dangote Industries Ltd",
    ...
    items: [
      {
        uuid: "mock-poi-001",
        inventoryItemId: "mock-inv-001",
        inventoryItemName: "Dangote Cement 50kg",
        inventoryItemUnit: "bag",
      ...
      {
        uuid: "mock-poi-002",
        inventoryItemId: "mock-inv-002",
        inventoryItemName: "Ready-Mix Concrete C30",
        inventoryItemUnit: "cu.m",
        inventoryItemType: "material",
        quantityOrdered: 30,
        unitPrice: 85000,
      },
  ...
  },
  {
    ...
    supplierName: "Lagos Steel Works",
    ...
    items: [
      {
        uuid: "mock-poi-003",
        inventoryItemId: "mock-inv-003",
        inventoryItemName: "Rebar 12mm x 12m",
        inventoryItemUnit: "pcs",
        inventoryItemType: "material",
        quantityOrdered: 200,
        unitPrice: 4400,
      },
      {
        uuid: "mock-poi-004",
        inventoryItemId: "mock-inv-004",
        inventoryItemName: "Rebar 16mm x 12m",
        inventoryItemUnit: "pcs",
        inventoryItemType: "material",
        quantityOrdered: 100,
        unitPrice: 7000,
  ...
  },
  {
    ...
    supplierName: "Abuja Electrical Supplies",
    ...
    items: [
      {
        uuid: "mock-poi-005",
        inventoryItemId: "mock-inv-005",
        inventoryItemName: "2.5mm Twin Cable (100m roll)",
        inventoryItemUnit: "roll",
        inventoryItemType: "material",
        quantityOrdered: 25,
        unitPrice: 28000,
      },
      {
        uuid: "mock-poi-006",
        inventoryItemId: "mock-inv-006",
        inventoryItemName: "Distribution Board 12-Way",
        inventoryItemUnit: "pcs",
...
export default function InventoryUpdatePage({ params }: InventoryUpdatePageProps) {
  ...
  const [libraryTemplate, setLibraryTemplate] = useState<ItemTemplate | null>(null);
  const [selectedPO, setSelectedPO] = useState<InventoryPO | null>(null);
  const [isPODetailOpen, setIsPODetailOpen] = useState(false);
  const [prefilledPOItems, setPrefilledPOItems] = useState<PrefilledLineItem[] | undefined>(undefined);
  ...
  const [actionModal, setActionModal] = useState<{
    ...
    type: "approve" | "reject";
    notes: string;
  }>({ open: false, request: null, type: "approve", notes: "" });
  ...
  const purchaseOrders: InventoryPO[] = posQuery.data?.items ?? MOCK_INVENTORY_POS;
  ...
  const deleteItemMutation = useMutation({
    mutationFn: (id: string) => inventoryService.deleteInventoryItem(projectUuid, id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["engineer", projectUuid, "inventory"] }),
  ...
  const approveRequestMutation = useMutation({
    mutationFn: ({ id, notes }: { id: string; notes: string }) =>
      inventoryService.approveMaterialRequest(projectUuid, id, notes),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["engineer", projectUuid, "material-requests"] });
      toast.success("Request approved");
      setActionModal({ open: false, request: null, type: "approve", notes: "" });
    },
    onError: (err: unknown) => toast.error(getErrorMessage(err)),
  });
  ...
  const rejectRequestMutation = useMutation({
    mutationFn: ({ id, notes }: { id: string; notes: string }) =>
      inventoryService.rejectMaterialRequest(projectUuid, id, { notes }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["engineer", projectUuid, "material-requests"] });
  ...
  const handleUpdateItem = (_updatedItem: InventoryItem) => {
    qc.invalidateQueries({ queryKey: ["engineer", projectUuid, "inventory"] });
    setIsAddModalOpen(false);
  };
  ...
  const handleApproveRequest = (request: MaterialRequest) => {
    ...
    setActionModal({ open: true, request, type: "approve", notes: "" });
  };

  const handleRejectRequest = (request: MaterialRequest) => {
    if (!request.id) return;
    setActionModal({ open: true, request, type: "reject", notes: "" });
  };

  const handleActionConfirm = () => {
    const { request, type, notes } = actionModal;
    if (!request?.id || (type === "reject" && !notes.trim())) return;
    if (type === "approve") {
      approveRequestMutation.mutate({ id: request.id, notes });
    } else {
      rejectRequestMutation.mutate({ id: request.id, notes });
    }
  };
  ...
  const handleViewPOFromRequest = async (request: MaterialRequest) => {
    if (!request.purchaseOrderId) return;
    try {
      const res = await purchaseOrderService.getPO(projectUuid, request.purchaseOrderId);
      const po = inventoryPOFromApi(res.data as Parameters<typeof inventoryPOFromApi>[0]);
      setSelectedPO(po);
      setIsPODetailOpen(true);
    } catch (err) {
  ...
  const handleCreatePOFromRequest = (request: MaterialRequest) => {
    setPrefilledPOItems([{
      ...
      inventoryItemName: request.itemName,
  ...
  const handleDeleteItem = (id: string) => { deleteItemMutation.mutate(id); };
  ...
  const handleViewAR = (_item: InventoryItem) => { setIsARScanOpen(true); };
  ...
  const handleEditFromDetail = () => {
    ...
    setIsDetailOpen(false);
    handleEditItem(selectedDetailItem);
  };
      ...
      <div className="space-y-6 pb-20 p-4 md:p-8 pt-8">
        ...
        <div className="bg-gray-100 rounded-xl p-1">
          <nav className="flex gap-1">
            ...
            <TabButton active={activeTab === "purchase-orders"} onClick={() => setActiveTab("purchase-orders")} icon={<ArrowRightLeft className="w-4 h-4" />} label="Purchase Orders" count={purchaseOrders.filter((po) => po.status === "draft" || po.status === "submitted").length || undefined} />
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">Create and track material requests for this project</p>
              </div>
            ...
            {requestsLoading ? (
              ...
              <RequestLogTable
                ...
                onCreatePO={handleCreatePOFromRequest}
                onViewPO={handleViewPOFromRequest}
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">Manage procurement for this project</p>
              </div>
              {perms.canCreatePO && (
                <button onClick={() => setIsCreatePOOpen(true)} className="flex items-center gap-2 px-4 py-2.5 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition-colors shadow-sm">
            ...
            {poLoading ? (
              ...
              <POTable
                ...
                onCreatePO={perms.canCreatePO ? () => setIsCreatePOOpen(true) : undefined}
                filters={poFilters}
        ...
        {/* Stock Counts Tab */}
        {activeTab === "stock-counts" && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-lg font-semibold text-gray-900">Stock Counts</h3>
                <p className="text-sm text-gray-500 mt-1">Physical count records and stock verification history</p>
              </div>
              {perms.canDoStockCount && (
                ...
                <>
                  ...
                  {filtered.length === 0 ? (
                    <div className="text-center py-12 bg-white rounded-lg border border-gray-200">
                      ...
                      <p className="text-sm text-gray-400 mt-1">{movSearch ? "Try a different search" : "Record your first stock movement"}</p>
                    </div>
                  ) : (
        ...
        {perms.canCreatePO && (
          <CreatePOModal
            ...
            onClose={() => { setIsCreatePOOpen(false); setPrefilledPOItems(undefined); }}
            projectId={projectUuid}
            inventoryItems={inventoryItems}
            prefilledItems={prefilledPOItems}
            onCreated={() => qc.invalidateQueries({ queryKey: ["engineer", projectUuid, "purchase-orders"] })}
          />
        )}
        ...
        <AnimatePresence>
          {isAddModalOpen && (
            <AddInventoryModal
              ...
              onClose={() => { setIsAddModalOpen(false); setEditingItem(null); setLibraryTemplate(null); }}
        ...
        <AnimatePresence>
          {actionModal.open && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4">
              <div className="fixed inset-0 bg-black/50" onClick={() => setActionModal((p) => ({ ...p, open: false }))} />
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative bg-white rounded-xl shadow-2xl w-full max-w-md p-6 space-y-4">
                <h3 className="text-lg font-semibold text-gray-900">{actionModal.type === "approve" ? "Approve Request" : "Reject Request"}</h3>
                <p className="text-sm text-gray-600">
                  {actionModal.type === "approve"
                    ? `Approve request for "${actionModal.request?.itemName}" by ${actionModal.request?.requestedByName}`
                    : `Reject request for "${actionModal.request?.itemName}" by ${actionModal.request?.requestedByName}`}
                </p>
                <textarea value={actionModal.notes} onChange={(e) => setActionModal((p) => ({ ...p, notes: e.target.value }))} placeholder={actionModal.type === "approve" ? "Add a note (optional)..." : "Reason (required)..."} rows={3} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 resize-none" autoFocus />
                <div className="flex justify-end gap-3">
                  <button onClick={() => setActionModal({ open: false, request: null, type: "approve", notes: "" })} className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors">Cancel</button>
                  <button
                    onClick={handleActionConfirm}
                    disabled={(actionModal.type === "reject" && !actionModal.notes.trim()) || approveRequestMutation.isPending || rejectRequestMutation.isPending}
                    className={`px-4 py-2 text-sm font-medium text-white rounded-lg disabled:opacity-50 transition-colors ${actionModal.type === "approve" ? "bg-emerald-600 hover:bg-emerald-700" : "bg-red-600 hover:bg-red-700"}`}
                  >
        ...
        <PODetailDrawer
          ...
          onStatusChange={() => qc.invalidateQueries({ queryKey: ["engineer", projectUuid, "purchase-orders"] })}
          onCreateChange={() => setShowCreateCO(true)}
        />

        {perms.canCreateChangeOrder && (
          <CreateChangeOrderModal
            isOpen={showCreateCO}
            onClose={() => setShowCreateCO(false)}
            purchaseOrders={selectedPO ? [selectedPO] : []}
            projectId={projectUuid}
            inventoryItems={inventoryItems.map((i) => ({ id: i.id, name: i.name, type: i.type, unit: i.unit || "pcs" }))}
            onCreated={(change) => { toast.success(`Change order ${change.changeNumber} submitted`); setShowCreateCO(false); }}
          />
        )}
