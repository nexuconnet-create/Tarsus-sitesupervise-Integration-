...
export default function HseQaQcPage({ params }: HseQaQcPageProps) {
  ...
  const createIncidentMutation = useMutation({
    mutationFn: async ({ data, files }: { data: CreateIncidentBody; files?: File[] }) => {
      ...
      if (files?.length && created.uuid) {
        await Promise.all(
          files.map((file) =>
            hseService.uploadEvidence(projectUuid ?? "", created.uuid, file)
          )
        );
      }
      return created;
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: hseKeys.incidents(projectUuid ?? "") });
    },
  });

  const createInspectionMutation = useMutation({
    mutationFn: (data: CreateInspectionBody) =>
      hseService.createInspection(projectUuid ?? "", data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: hseKeys.inspections(projectUuid ?? "") });
    },
  });

  // ─── API-backed state for remaining tabs ──────────────────────────────────
  ...
  // ─── Handlers ──────────────────────────────────────────────────────────────
  const handleIncidentSubmit = (data: CreateIncidentBody, files?: File[]) => {
    createIncidentMutation.mutate({ data, files });
  };

  const handleInspectionSubmit = (data: CreateInspectionBody) => {
  ...
  const handlePTWSubmit = (data: CreatePermitBody) => createPermitMutation.mutate(data);
  const handleNCRSubmit = (data: CreateNCRBody) => createNCRMutation.mutate(data);
  const handleRiskSubmit = (data: CreateRiskAssessmentBody) => createRiskMutation.mutate(data);
  const handleToolboxSubmit = (data: CreateToolboxTalkBody) => createToolboxMutation.mutate(data);
  const handleTrainingSubmit = (data: CreateTrainingBody) => createTrainingMutation.mutate(data);

  // ─── Color helpers (API enum values) ──────────────────────────────────────
  ...
  const getNCRStatusColor = (status: string) => {
    switch (status) {
      case "OPEN": return "bg-red-100 text-red-700";
  ...
  const getPTWStatusColor = (status: string) => {
      ...
      <div className="space-y-6 pb-20 p-4 md:p-8 pt-8">
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-200">
          <div className="bg-[#021422] text-white p-4">
            <h2 className="text-sm font-semibold tracking-widest uppercase">Safety</h2>
          </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              <div className="p-4 max-h-[500px] overflow-y-auto">
                    ...
                    {incidentsLoading ? (
                      ...
                      incidents.map((incident) => (
                        <div
                          ...
                          onClick={() => { setSelectedIncident(incident); setIsIncidentModalOpen(true); }}
                        >
                    ...
                    {inspectionsLoading ? (
                      ...
                      inspections.map((inspection) => (
                        <div
                          key={inspection.uuid}
                          className="border border-gray-100 rounded-xl p-4 hover:shadow-sm transition-shadow cursor-pointer"
                          onClick={() => { setSelectedInspection(inspection); setIsInspectionModalOpen(true); }}
                        >
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex-1">
                              <p className="font-semibold text-[#021422]">{inspection.inspection_type}</p>
                              <div className="flex items-center gap-4 mt-2 text-xs text-gray-500">
                ...
                {activeTab === "ncrs" && (
                  <div className="space-y-3">
                    {ncrsLoading ? (
                      [1, 2, 3].map((i) => <div key={i}>{loadingRow}</div>)
                    ) : ncrs.length === 0 ? (
                      <p className="text-center text-gray-500 py-8">No NCRs recorded</p>
                    ) : (
                      ncrs.map((ncr) => (
                        <div key={ncr.uuid} className="border border-gray-100 rounded-xl p-4 hover:shadow-sm transition-shadow cursor-pointer" onClick={() => { setSelectedNCR(ncr); setIsNCRModalOpen(true); }}>
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-1">
                ...
                {activeTab === "ptws" && (
                  <div className="space-y-3">
                    {ptwsLoading ? (
                      ...
                      ptws.map((ptw) => (
                        <div key={ptw.uuid} className="border border-gray-100 rounded-xl p-4 hover:shadow-sm transition-shadow cursor-pointer" onClick={() => { setSelectedPTW(ptw); setIsPTWModalOpen(true); }}>
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-1">
                                <FileText size={16} className="text-gray-400" />
                                <span className="font-semibold text-[#021422]">{PTW_TYPE_LABELS[ptw.type]}</span>
                              </div>
                  ...
                  <div className="space-y-3">
                    {riskLoading ? (
                      ...
                      riskAssessments.map((risk) => {
                        ...
                        return (
                          <div key={risk.uuid} className="border border-gray-100 rounded-xl p-4 hover:shadow-sm transition-shadow cursor-pointer" onClick={() => { setSelectedRisk(risk); setIsRiskModalOpen(true); }}>
                            <div className="flex items-start justify-between gap-4">
                              <div className="flex-1">
                                <div className="flex items-center gap-2 mb-1">
                                ...
                                <p className="font-semibold text-[#021422]">{risk.description}</p>
                                {controlItems.length > 0 && (
                                  <div className="flex flex-wrap gap-1 mt-2">
                                    {controlItems.slice(0, 3).map((control, i) => (
                                      <span key={i} className="px-2 py-0.5 bg-gray-100 text-gray-600 rounded text-xs">
                                        {control}
                                      </span>
                                    ))}
                                    {controlItems.length > 3 && (
                                      <span className="px-2 py-0.5 bg-gray-100 text-gray-500 rounded text-xs">+{controlItems.length - 3} more</span>
                                    )}
                                  </div>
                                )}
                                <p className="text-xs text-gray-500 mt-2">{risk.date_assessed ? new Date(risk.date_assessed).toLocaleDateString() : ""}</p>
                              </div>
                            </div>
                          </div>
                        );
                  ...
                  <div className="space-y-3">
                    {toolboxLoading ? (
                      ...
                      toolboxTalks.map((talk) => {
                        ...
                        return (
                          <div key={talk.uuid} className="border border-gray-100 rounded-xl p-4 hover:shadow-sm transition-shadow cursor-pointer" onClick={() => { setSelectedTalk(talk); setIsToolboxModalOpen(true); }}>
                            <div className="flex items-start justify-between gap-4">
                              <div className="flex-1">
                                <div className="flex items-center gap-2 mb-1">
                                ...
                                {attendees.length > 0 && (
                                  <div className="flex flex-wrap gap-1 mt-2">
                                    ...
                                    {attendees.length > 4 && (
                                      <span className="px-2 py-0.5 bg-gray-100 text-gray-500 rounded text-xs">+{attendees.length - 4} more</span>
                                    )}
                                  </div>
                                )}
                  ...
                  <div className="space-y-3">
                    {trainingLoading ? (
                      ...
                      training.map((t) => {
                        ...
                        return (
                          <div key={t.uuid} className="border border-gray-100 rounded-xl p-4 hover:shadow-sm transition-shadow cursor-pointer" onClick={() => { setSelectedTraining(t); setIsTrainingModalOpen(true); }}>
                            <div className="flex items-start justify-between gap-4">
                              <div className="flex-1">
                                <div className="flex items-center gap-2 mb-1">
                                  <GraduationCap size={16} className="text-gray-400" />
                                  <span className="font-semibold text-[#021422]">{t.programme_name}</span>
                                </div>
