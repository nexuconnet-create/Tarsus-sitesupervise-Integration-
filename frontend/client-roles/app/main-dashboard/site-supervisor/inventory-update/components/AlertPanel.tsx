...
import { getLowStockItems } from "@/lib/mockData/inventory";
...
const AlertPanel = ({ items }: AlertPanelProps) => {
  const [isExpanded, setIsExpanded] = useState(false);
  const alertItems = items || getLowStockItems();
...
<div className="flex items-center gap-3">
  <StockBadge status="low" size="sm" />
                <button className="px-3 py-1.5 text-sm font-medium text-white bg-[#021422] rounded-lg hover:bg-gray-800 transition-colors">
                  Reorder
                </button>
              </div>
            </div>
