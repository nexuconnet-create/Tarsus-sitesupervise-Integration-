...
import { X, Plus, Search, Users, CheckCircle2, Clock, XCircle, AlertCircle } from "lucide-react";
...
const StatusBadge = ({ status }: { status: "confirmed" | "pending" | "rejected" }) => {
  if (status === "pending") {
    return (
...
interface DailyWorkerModalProps {
  ...
  onSave: (date: string, workerIds: string[]) => void;
}
...
export default function DailyWorkerModal({
  ...
  const [workerIds, setWorkerIds] = useState<string[]>(assignedWorkerIds);
  const [showPool, setShowPool] = useState(false);
  const [search, setSearch] = useState("");
  const [selectedPoolIds, setSelectedPoolIds] = useState<Set<string>>(new Set());

  const assignedWorkers = useMemo(() => {
    const filteredCrewWorkers = crewWorkers.filter((w) => workerIds.includes(w.id));
    const filteredPoolWorkers = availablePool.filter((w) => workerIds.includes(w.id));
    return [...filteredCrewWorkers, ...filteredPoolWorkers];
  }, [crewWorkers, availablePool, workerIds]);

  const poolWorkers = useMemo(() => {
    return availablePool.filter((w) => {
      if (workerIds.includes(w.id)) return false;
      if (!search.trim()) return true;
      const q = search.toLowerCase();
      return w.name.toLowerCase().includes(q) || w.trade.toLowerCase().includes(q);
    });
  }, [availablePool, workerIds, search]);

  const hasChanges = JSON.stringify(workerIds.sort()) !== JSON.stringify(assignedWorkerIds.sort());

  const handleRemove = (workerId: string) => {
    setWorkerIds((prev) => prev.filter((id) => id !== workerId));
  };
  ...
  const handleAddSelected = () => {
    if (selectedPoolIds.size > 0) {
      setWorkerIds((prev) => [...prev, ...Array.from(selectedPoolIds)]);
          ...
          <div className="px-6 py-4 border-b border-gray-100">
            <div className="flex items-start justify-between">
              <div>
                ...
                <h3 className="text-base font-bold text-gray-900 mt-0.5">{displayDate}</h3>
                <div className="mt-2">
            ...
            {assignedWorkers.length > 0 ? (
              <div className="space-y-1">
                {assignedWorkers.map((worker) => (
                  <div
                    key={worker.id}
                    className={`flex items-center justify-between px-3 py-2.5 rounded-lg bg-gray-50 group ${status === "pending" ? "" : ""}`}
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-gray-400">{worker.memberId} • {worker.trade}</p>
                      </div>
                    </div>
                    {status !== "pending" && (
                      <button
                      ...
                      return (
                        <button
                          ...
                          className={`w-full flex items-center justify-between px-3 py-2 transition-colors text-left border-l-2 ${
                            isSelected ? "bg-gray-100 border-gray-900" : "border-transparent hover:bg-gray-50"
                          }`}
                        >
                          <div className="flex items-center gap-2">
                            <div
                            ...
                            <div>
                              <p className="text-xs font-medium text-gray-900">{worker.name}</p>
                              <p className="text-[10px] text-gray-400">{worker.trade}</p>
                            </div>
                          </div>
                        </button>
                      );
            ...
            {status === "pending" && (
              <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                <p className="text-xs text-yellow-700">
                  This day is pending approval. You cannot make changes until it is approved or rejected.
                </p>
              </div>
            )}
          </div>
