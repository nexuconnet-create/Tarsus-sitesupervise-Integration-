...
export default function ScheduleBulkMarkModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Mock: Store in localStorage as mock attendance
      const existingMock = JSON.parse(
        localStorage.getItem("MOCK_SCHEDULE_ATTENDANCE") || "{}",
      );
      const scheduleKey = `schedule_${schedule.id}`;
      existingMock[scheduleKey] = {
      ...
      localStorage.setItem(
        "MOCK_SCHEDULE_ATTENDANCE",
        JSON.stringify(existingMock),
      );
      toast.success("Schedule attendance marked successfully");

      // TODO: Replace with API call when backend is ready
      // await staffService.bulkMarkScheduleAttendance({
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-6xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              {schedule.title} â€” {formatDate(schedule.durationFrom)} to{" "}
        ...
        <div className="flex items-center justify-between px-6 py-3 bg-gray-50 border-b border-gray-100 shrink-0">
          ...
          <div className="text-[10px] text-gray-500">
            <span className="font-bold">{allWorkers.length}</span> workers Ã—{" "}
