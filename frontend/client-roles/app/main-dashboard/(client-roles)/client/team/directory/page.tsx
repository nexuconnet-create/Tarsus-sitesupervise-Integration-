      ...
      <div className="bg-white border-b border-slate-200 px-8 py-6 flex flex-col md:flex-row md:items-center justify-between gap-4 shrink-0">
        <div>
          <h1 className="text-2xl font-extrabold text-[#021422] flex items-center gap-3 uppercase tracking-wider">
            <Contact className="text-emerald-500" size={24} />
            Contact Directory
          </h1>
          ...
          <div className="flex-1 space-y-10">
            {letters.map((letter) => (
              <div key={letter} id={`letter-${letter}`} className="scroll-mt-6">
                <h2 className="text-2xl font-black text-slate-300 mb-4 pl-2 border-b-2 border-slate-200 pb-2">{letter}</h2>
                <div className="grid gap-3">
                  {groupedContacts[letter].map((contact) => (
                    <div key={contact.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm hover:border-blue-300 transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-4 group">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <Link href={`/main-dashboard/client/communication/messages`} className="p-2 text-slate-400 hover:bg-emerald-50 hover:text-emerald-600 rounded-lg transition-colors" title="Call via Messages">
                          <Phone size={18} />
                        </Link>
                        ...
                        <Link href={`/main-dashboard/client/communication/messages`} className="p-2 text-slate-400 hover:bg-slate-50 hover:text-slate-600 rounded-lg transition-colors" title="Message">
