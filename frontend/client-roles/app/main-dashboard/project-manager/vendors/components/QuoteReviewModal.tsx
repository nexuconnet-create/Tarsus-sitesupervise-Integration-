...
interface QuoteReviewModalProps {
  ...
  onQuoteRejected?: (quoteId: string, reqId: string) => void;
}
...
const QuoteReviewModal: React.FC<QuoteReviewModalProps> = ({
  ...
  const handleRejectQuote = async (quoteId: string) => {
    ...
    if (onQuoteRejected && requisition) {
      onQuoteRejected(quoteId, requisition.id);
    }
  };
                          ...
                          {quote.items.map((item, idx) => (
                            <div key={idx} className="flex justify-between text-sm">
                              <span className="text-gray-600">
                                {item.quantity} {item.unit} &times; {item.name}
                              </span>
