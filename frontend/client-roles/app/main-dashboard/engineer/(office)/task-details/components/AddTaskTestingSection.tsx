...
export default function AddTaskTestingSection({ testFields, appendTest, removeTest }: AddTaskTestingSectionProps) {
  const [categories, setCategories] = useState(TEST_CATEGORIES);
  const [selectedTestType, setSelectedTestType] = useState(TEST_CATEGORIES[0]?.testType || "");
  ...
  const selectedCategory = categories.find((c) => c.testType === selectedTestType);
      ...
      <div className="bg-gray-50 rounded-xl p-4 space-y-3">
        <div className="flex items-center gap-3">
          ...
          <div className="relative flex-1 max-w-sm">
            ...
            >
              {categories.map((cat) => (
                <option key={cat.testType} value={cat.testType}>{cat.testType}</option>
              ))}
              <option value="__ADD_NEW__" className="font-semibold text-[#021422]">
                + Add New Category
              </option>
            </select>
            <ChevronDown
...
<div className="flex gap-3 pt-2">
      ...
      {testFields.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 bg-gray-50 rounded-xl border border-dashed border-gray-200 text-gray-400">
          ...
          <p className="text-xs text-gray-400 mt-1">Select a category and click &quot;Add Test&quot;</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                <th className="text-left px-4 py-3 text-xs font-bold uppercase text-gray-500 tracking-wider">Test Name</th>
                <th className="text-left px-4 py-3 text-xs font-bold uppercase text-gray-500 tracking-wider">Category</th>
                <th className="text-left px-4 py-3 text-xs font-bold uppercase text-gray-500 tracking-wider">Date</th>
                <th className="text-left px-4 py-3 text-xs font-bold uppercase text-gray-500 tracking-wider">Production Date</th>
                <th className="text-left px-4 py-3 text-xs font-bold uppercase text-gray-500 tracking-wider">Company</th>
                <th className="w-12" />
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {testFields.map((test, index) => (
                <tr key={test.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 font-semibold text-[#021422]">{test.label}</td>
                  <td className="px-4 py-3 text-sm text-gray-500">{test.type}</td>
                  <td className="px-4 py-3 text-sm text-gray-400">{test.dateAdded}</td>
                  <td className="px-4 py-3 text-sm text-gray-400">{test.productionDate || "â€”"}</td>
                  <td className="px-4 py-3 text-sm text-gray-400">{test.companyName || "â€”"}</td>
